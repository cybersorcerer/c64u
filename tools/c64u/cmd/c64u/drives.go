		...
		if jsonOut {
			...
			for _, driveData := range drives {
				...
				for driveName, driveInfo := range driveMap {
					...
					// Print drive header with status icon
					statusIcon := "●"
					statusText := "Enabled"
					...
					formatter.PrintHeader(fmt.Sprintf("%s %s (%s)", statusIcon, driveName, statusText))
					fmt.Println()
